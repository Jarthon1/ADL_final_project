...
def process_file(series_file):
    ...
    try:
        ...
        elif cohort in ['isruc','test_cohort']:
            with open(file_hypnogram, 'r') as hyp_file:
    ...
    keep_idx = []
    if cohort in ['wsc', 'ssc', 'isruc', 'test_cohort']:
...
def process_cohort(paths_cohort, name_cohort):
    ...
    if name_cohort in ['isruc','test_cohort']:
        baseDir = [os.path.split(edf[:edf.find('subgroup')])[0]
                   for edf in list_edf]
        list_fileID = [fid[fid.find('data')+5:-4] for fid in list_edf]
        print(list_edf)
        print(list_fileID)
    else:
    ...
    if name_cohort == 'wsc' or name_cohort == 'ssc':
        ...
        hyp_IDs = [hypID[hypID.find('data/')+5:-6]
