...
class GraphConvpoolMPNN_block_v6(nn.Module):
    def __init__(self, input_dim, output_dim, num_sensors, time_length, time_window_size, stride, decay, pool_choice):
        ...
        # Add attention layer after graph convolution and pooling
        self.attention_layer = AttentionLayer(output_dim, output_dim // 2, output_dim)
    ...
    def forward(self, input):
        ...
        # Apply attention to the output features after pooling
        X_output = tr.transpose(X_output, 1, 2)  # Reshape to (bs, num_sensors, num_windows, output_dim)
        X_output = tr.reshape(X_output, [bs * num_sensors, num_windows, self.output_dim])
        X_output = self.attention_layer(X_output)
        X_output = tr.reshape(X_output, [bs, num_sensors, num_windows, self.output_dim])
        X_output = tr.transpose(X_output, 1, 2)  # Reshape back to (bs, num_windows, num_sensors, output_dim)
        
        return X_output

#uses attention_weighted_pooling
class GraphConvpoolMPNN_block_v7(nn.Module):
    def __init__(self, input_dim, output_dim, num_sensors, time_length, time_window_size, stride, decay, pool_choice):
        super(GraphConvpoolMPNN_block_v7, self).__init__()
        self.time_window_size = time_window_size
        self.stride = stride
        self.output_dim = output_dim
        ...
        # Add attention layer for weighted pooling
        self.attention_layer = nn.Linear(output_dim, 1)
    ...
    def forward(self, input):
        ...
        attention_weights = self.attention_layer(X_output)
        attention_weights = tr.softmax(attention_weights, dim=2)
        
        # Perform weighted pooling
        X_output = tr.sum(X_output * attention_weights, dim=2)
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, node_features):
        ...
        query = self.query_layer(node_features)  # (batch_size, num_nodes, hidden_dim)
        key = self.key_layer(node_features)  # (batch_size, num_nodes, hidden_dim)
        value = self.value_layer(node_features)  # (batch_size, num_nodes, output_dim)
        
        # Compute attention scores
        attention_scores = tr.matmul(query, key.transpose(-2, -1))  # (batch_size, num_nodes, num_nodes)
        attention_scores = attention_scores / np.sqrt(self.hidden_dim)
        
        # Compute attention weights
        attention_weights = self.softmax(attention_scores)  # (batch_size, num_nodes, num_nodes)
        
        # Apply attention weights to value vectors
        attended_features = tr.matmul(attention_weights, value)  # (batch_size, num_nodes, output_dim)
